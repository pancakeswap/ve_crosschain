...
describe("VeCakeProxyV2", () => {
    ...
    describe("Normal user lock cake in VECake", () => {
        ...
        it("Create lock and sync with extra fee see if it will refund", async function () {
            await CakeTokenSC.connect(user2).approve(VECakeSC.address, ethers.constants.MaxUint256);

            let now = (await time.latest()).toString();
            let OneYear = BigNumber.from(now).add(YEAR);

            await VECakeSC.connect(user2).createLock(ethers.utils.parseUnits("1000"), OneYear);
            ...
            let balanceABefore2 = await ethers.provider.getBalance(user2.address);
            ...
            let balanceAfter2 = await ethers.provider.getBalance(user2.address);

            let diff1 = balanceABefore2.sub(balanceAfter2);

            console.log('spend fee 1st: ', ethers.utils.formatEther(diff1));

            balanceABefore2 = await ethers.provider.getBalance(user2.address);
            ...
            let diff2 = balanceABefore2.sub(balanceAfter2);

            console.log('spend fee 2nd: ', ethers.utils.formatEther(diff2));

            expect(diff1.sub(diff2)).to.lte(ethers.utils.parseUnits("0.001"));
        });
        ...
        it("Increase Lock Amount", async function () {
            ...
            let nativeFee4 = 0;
            ;[nativeFee4] = await pancakeVeSenderV2SC.connect(user4).getEstimateGasFees(eidB, 0);
            console.log('nativeFee2: ', nativeFee4);
            ...
            // sendSyncMsg
            await pancakeVeSenderV2SC.sendSyncMsg(
                eidB,
                user4.address,
                true,
                true,
    ...
    describe("withdraw", () => {
        ...
        it("Withdraw after lock expired", async function () {
            ...
            ;[nativeFee4] = await pancakeVeSenderV2SC.connect(user4).getEstimateGasFees(eidB, 600000);
            console.log('nativeFee4: ', nativeFee4);
