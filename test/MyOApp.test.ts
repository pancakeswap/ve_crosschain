...
describe('MyOApp Test', function () {
    ...
    beforeEach(async function () {
        ...
        // Transfer ownership
        await myOAppA.connect(admin).transferOwnership(ownerA.address)
        await myOAppB.connect(admin).transferOwnership(ownerB.address)
    ...
    it('should send a message to each destination OApp', async function () {
        ...
        let balanceABefore1 = await ethers.provider.getBalance(ownerA.address)

        // Execute send operation from myOAppA
        await myOAppA.connect(ownerA).send(eidB, 'Test message.', options, {
            value: (nativeFee).toString()
        })

        let balanceAfter1 = await ethers.provider.getBalance(ownerA.address)

        console.log('spend: ', balanceABefore1.sub(balanceAfter1).toString())

        let balanceABefore2 = await ethers.provider.getBalance(ownerA.address)

        // Execute send operation from myOAppA
        await myOAppA.connect(ownerA).send(eidB, 'Test message.', options, {
            value: (nativeFee*10).toString()
        })

        let balanceAfter2 = await ethers.provider.getBalance(ownerA.address)

        console.log('spend: ', balanceABefore2.sub(balanceAfter2).toString())
